...
impl GeneralPermissionConsts {
    pub const MANAGE_CAMPSITE: u64 = 1 << 0;
    pub const MANAGE_BONFIRES: u64 = 1 << 1;
    pub const MANAGE_TENTS: u64 = 1 << 3;
    pub const MANAGE_ROLES: u64 = 1 << 4;
    pub const GIVE_ROLES: u64 = 1 << 5;
    pub const MUTE_MEMBERS: u64 = 1 << 6;
    pub const KICK_MEMBERS: u64 = 1 << 7;
    pub const BAN_MEMBERS: u64 = 1 << 8;
    pub const MANAGE_SELF_IDENTITY: u64 = 1 << 9;
    pub const MANAGE_OTHERS_IDENTITY: u64 = 1 << 10;
    pub const CREATE_INVITES: u64 = 1 << 11;
    pub const MANAGE_INVITES: u64 = 1 << 12;
    pub const MAX: u64 = u64::MAX ^ (u64::MAX >> 12 << 12);
}
pub struct ContentPermissionConsts { }
impl ContentPermissionConsts {
    pub const VIEW_CONTENT: u64 = 1 << 0;
    pub const CREATE_CONTENT: u64 = 1 << 1;
    pub const PIN_CONTENT: u64 = 1 << 2;
    pub const MANAGE_CONTENT: u64 = 1 << 3;
    pub const MENTION_EVERYONE: u64 = 1 << 4;
    pub const CREATE_PRIVATE_CONTENT: u64 = 1 << 5;
    pub const MAX: u64 = u64::MAX ^ (u64::MAX >> 5 << 5);
}

pub async fn has_role_permissions(member: &CampsiteMember, general_perms: u64, content_perms: u64) -> Result<bool, XRPCError> {
...
pub async fn has_any_role_permissions(member: &CampsiteMember, general_perms: u64) -> Result<bool, XRPCError> {
    let member_role_ids: &Vec<Uuid> = &member.roles.iter().filter_map(|&x| x).collect::<Vec<Uuid>>();
    let roles = &get_specific_roles(member_role_ids)?;
...
pub async fn has_role_perms_or_owner(campsite: &Campsite, member: &CampsiteMember, general_perms: u64, content_perms: u64) -> Result<bool, XRPCError> {
    if campsite.owner == member.user_id { Ok(true) } else { has_role_permissions(member, general_perms, content_perms).await }
}
pub async fn has_any_role_perms_or_owner(campsite: &Campsite, member: &CampsiteMember, general_perms: u64) -> Result<bool, XRPCError> {
    if campsite.owner == member.user_id { Ok(true) } else { has_any_role_permissions(member, general_perms).await }
}
pub async fn has_role_perms_from_roles_or_owner(campsite: &Campsite, member: &CampsiteMember, roles: &Vec<CampsiteRole>, general_perms: u64, content_perms: u64) -> Result<bool, XRPCError> {
...
pub async fn has_leveled_perms_or_owner(campsite: &Campsite, bonfire_id: &str, category_id: Option<Uuid>, tent_id: Option<Uuid>, member: &CampsiteMember, general_perms: u64, content_perms: u64) -> Result<bool, XRPCError> {
...
pub async fn has_full_leveled_perms(campsite_id: &str, bonfire_id: &str, category_id: Option<Uuid>, tent_id: Option<Uuid>, member: &CampsiteMember, general_perms: u64, content_perms: u64) -> Result<bool, XRPCError> {
...
pub async fn has_full_leveled_perms_from_roles(campsite_id: &str, bonfire_id: &str, category_id: Option<Uuid>, tent_id: Option<Uuid>, roles: &Vec<CampsiteRole>, member: &CampsiteMember, general_perms: u64, content_perms: u64) -> Result<bool, XRPCError> {
    let member_roles: &Vec<Uuid> = &member.roles.iter().filter_map(|&x| x).collect();
...
pub async fn aggregate_leveled_permission(campsite_id: &str, bonfire_id: &str, category_id: Option<Uuid>, tent_id: Option<Uuid>, actor: &str, role_ids: &Vec<Uuid>, general_perms: u64, content_perms: u64) -> Result<(Vec<CampsitePermission>, PermissionState, PermissionState), XRPCError> {
