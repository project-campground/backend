...
use campground_lexicon::gg::campground::permission::{
    PermissionsDictionary, PermissionsStateDictionary,
};
use reqwest::Client;
use rocket::State;
...
use crate::{
    realtime::{
        data::{ReactiveSubject, ReactiveSubjectData},
        frames::{SocketDataFrame, SocketErrorFrame, SocketFrameSerializer, SocketFrameType},
        messages::SocketAuthFrame,
    },
    xrpc::auth::validate_jwt,
};
...
pub async fn get_did_from_auth(
    did_document_storage: &State<LruDidDocumentStorage>,
    client: &State<Client>,
    init_message: &Message,
) -> Result<Option<String>, (Option<Vec<u8>>, ws::Message)> {
    match init_message {
        ws::Message::Binary(bytes) => {
            ...
            if auth_data.is_err() {
                ...
                let (message, close_message) = SocketErrorFrame::from_error_message(
                    "AuthenticationParsingError",
                    "Could not parse binary auth CBOR",
                );
            ...
            if auth_data.op != SocketFrameType::Auth {
                let (message, close_message) = SocketErrorFrame::from_error_message(
                    "AuthenticationBadMessage",
                    "Expected authentication op to be 0",
                );
            ...
            if let Some(payload) = auth_data.payload {
                // Get actor
                let validated_jwt =
                    validate_jwt(&payload.service_auth, did_document_storage, client).await;
        ...
        _ => {
            let (message, close_message) = SocketErrorFrame::from_error_message(
                ...
                "Expected a binary authentication message",
            );
    ...
    TFn: Fn(Vec<u8>) -> ReactiveSubjectData,
{
    ...
    let response = SocketDataFrame::<TData>::new(data_type.to_string(), payload);
    let binary = response.binary();

    if let Ok(binary) = binary {
        event_subject.inner.clone().next(on_next(binary));
    }
}

pub fn event_next_campsite<T>(
    event_subject: &State<ReactiveSubject>,
    campsite_id: &String,
...
pub fn event_next_campsite_global<T>(
    event_subject: &State<ReactiveSubject>,
    campsite_id: &String,
    data_type: &str,
    payload: &T,
