...
use common::{GetRecordResponse, ResponseResult, fetch_record, fetch_record_list};
use diesel::dsl::sql;
...
use crate::database::{establish_connection, models::appview::Actor, models::appview::ProfilePost};
use crate::schema::appview::profile_post;
use crate::{
    database::actors::get_actor, helpers::api::handle_all_db_errors, xrpc::error::XRPCError,
};
...
pub fn resolve_post_uri(uri: &str) -> Result<(String, String, Option<String>), &'static str> {
    let post_uri = if uri.starts_with("at://") {
        uri.to_string()
    } else {
        format!("at://{}", uri)
    };
    ...
    let uri_formatted = match split_uri[..] {
        [_, _, did, "gg.campground.profile.post", post_tid] => format!(
            "at://{did}/gg.campground.profile.post/{post_tid}",
            did = did,
            post_tid = post_tid
        ),
    ...
    Ok((
        ...
        if split_uri.len() < 5 {
            ...
            Some(split_uri[4].to_string())
        },
    ))
}

pub async fn fill_profile_posts_with_records(
    client: &Client,
    did_document_storage: &LruDidDocumentStorage,
    author_did: &str,
    parent_uri: Option<String>,
    optional_parent: bool,
    posts: &mut Vec<ProfilePost>,
) -> Result<(), XRPCError> {
    ...
    let post_records = fetch_record_list::<ProfilePostRecord>(
        ...
        .await
        .inspect_err(|err| println!("Failed to fetch post records: {:?}", err))
    ...
    match post_records {
        ...
        ResponseResult::Ok(post_records) => {
            ...
            let posts_modified: &Vec<ProfilePost> = &post_records
                .records
                .iter()
                .filter(|x| {
                    posts.iter().find(|y| y.uri == x.uri).map_or(
                        false,
                        // TODO Likely to be modified when embeds exist
                        |y| {
                            x.value.content.is_some()
                                && y.content != x.value.content.clone().unwrap()
                        },
                    )
                })
                .map(|x| db_post_from_only_record(x))
            ...
            posts.sort_by(|a, b| {
                NaiveDateTime::parse_from_str(b.indexed_at.as_str(), fmt)
                    .unwrap()
                    .cmp(&NaiveDateTime::parse_from_str(a.indexed_at.as_str(), fmt).unwrap())
            });
...
pub async fn get_single_profile_post<T>(
    client: &Client,
    did_document_storage: &LruDidDocumentStorage,
    post_query: Result<T, diesel::result::Error>,
    author_did: &str,
    post_tid: &str,
    _fn: fn(ProfilePost) -> T,
) -> Result<(Actor, T), XRPCError> {
    let author_actor = get_actor(client, did_document_storage, author_did)
        .await
        .inspect_err(|err| println!("Error fetching actor: {}", err))
        .map_err(|_| XRPCError::InternalServerError)?;
    ...
    match post_query {
        ...
        Err(NotFound) => {
            ...
            .await
            .inspect_err(|err| println!("Error fetching post record: {}", err))
            .map_err(|_| XRPCError::InternalServerError)?;

            match post_record {
                ResponseResult::Ok(post_record) => {
                    let profile_post = insert_post_into_db(&post_record, author_did).await?;
        
                    Ok((author_actor.clone(), _fn(profile_post)))
                },
                ResponseResult::Error { error, message: _ } => {
                    match error.as_str() {
                        "InvalidRequest" | "RecordNotFound" => Err(XRPCError::NotFound),
                        _ => Err(XRPCError::InternalServerError),
                    }
                }
            }
        }
        Err(_) => Err(XRPCError::InternalServerError),
    }
}

pub async fn delete_post_record_from_db(
    uri: &str,
    parent_uri: Option<String>,
    ...
    if parent_uri.is_some() {
        diesel::update(profile_post::table)
            .filter(profile_post::uri.eq(parent_uri.unwrap()))
            .set(
                profile_post::replies.eq(sql("array_remove(replies, ")
                    .bind::<VarChar, _>(uri)
                    .sql(")")),
            )
            .execute(&mut conn)
    ...
    let deleted_count = diesel::delete(profile_post::table.filter(profile_post::uri.eq(uri)))
        .execute(&mut conn)
...
fn db_post_from_only_record(post_record: &GetRecordResponse<ProfilePostRecord>) -> ProfilePost {
    let x_author_did = post_record
        .uri
        .split("/")
        .skip(2)
        .next()
        .map_or("did:plc:null", |x| x);
    db_post_from_record(post_record, x_author_did)
}

fn db_post_from_record(
    post_record: &GetRecordResponse<ProfilePostRecord>,
    author_did: &str,
) -> ProfilePost {
...
async fn insert_post_list_into_db(posts: Vec<ProfilePost>) -> Result<(), XRPCError> {
    let mut conn = establish_connection().unwrap();
    ...
    let parents: Vec<String> = posts
        .clone()
        .iter()
        // Don't care about ones without parents
        .filter(|x| x.parent_uri.is_some())
        // Stringed tuple
        .map(|x| format!("{};{}", x.uri.clone(), x.parent_uri.clone().unwrap()))
        .collect();
    ...
    let reply_update_sql =
        sql("(SELECT array_agg(split_part(unnest, ';', 1)) FROM ( SELECT unnest(")
            .bind::<Array<VarChar>, _>(parents.clone())
            .sql(") ) WHERE split_part(unnest, ';', 2) = \"appview\".\"profile_post\".\"uri\")");

    diesel::update(profile_post::table)
        .filter(
            profile_post::uri.eq_any(
                parents
                    .iter()
                    .map(|x| x.split(";").collect::<Vec<&str>>()[1]),
            ),
        )
        .set(profile_post::replies.eq(profile_post::replies.concat(reply_update_sql)))
        .execute(&mut conn)
        .map_err(handle_all_db_errors)?;
...
async fn insert_post_into_db(
    post_record: &GetRecordResponse<ProfilePostRecord>,
    author_did: &str,
) -> Result<ProfilePost, XRPCError> {
    let mut conn = establish_connection().unwrap();
    ...
    diesel::update(profile_post::table)
        .filter(profile_post::uri.eq(parent_uri.clone()))
        .set(profile_post::replies.eq(profile_post::replies.concat(vec![post_record.uri.clone()])))
        .execute(&mut conn)
        .map_err(handle_all_db_errors)?;
...
async fn update_post_list_in_db(posts: Vec<ProfilePost>) -> Result<(), XRPCError> {
    ...
    // Stringed tuple inserted posts and their parents to keep a list of parents to update
    let update_tuple_string: Vec<String> = posts
        .clone()
        .iter()
        // Stringed tuple
        .map(|x| format!("{};{}", x.uri.clone(), x.content.clone()))
        .collect();
    ...
    diesel::update(profile_post::table)
        ...
        .set(profile_post::content.eq(content_update_sql))
        .execute(&mut conn)
        .map_err(handle_all_db_errors)?;
