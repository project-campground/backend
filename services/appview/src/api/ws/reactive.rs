...
use diesel::{BoolExpressionMethods, ExpressionMethods, NullableExpressionMethods, QueryDsl, RunQueryDsl, WindowExpressionMethods, dsl::count};
...
pub async fn on_rocket_message(msg: Message, ws_actor: &Option<String>, _actor_campsites: &mut Vec<Option<String>>, current_campsite: &mut Option<String>, current_membership: &mut Option<CampsiteMember>, permissions: &mut CampsiteMemberPermissions) -> WebSocketOutput {
    match msg {
        ws::Message::Ping(payload) => 
            WebSocketOutput::RocketMessage(
        ...
        ws::Message::Binary(bytes) => {
            ...
            if data.is_err() {
                ...
                return WebSocketOutput::MessagedClose("ParsingError".to_string(), "Could not parse binary CBOR".to_string());
            }
                    ...
                    match data.payload {
                        SocketInFramePayload::View(view) => {
                            ...
                            *current_campsite = if view.campsite == "" { None } else { Some(view.campsite) };
                            ...
                            if let Some(campsite_id) = current_campsite && let Some(ws_actor) = ws_actor {
                                *current_membership = get_campsite_member(campsite_id, ws_actor).ok();
    
                                *permissions = MEMBER_PERMS_DEFAULT.clone();
                                let roles = try_or_continue!(get_roles_from_db(campsite_id).ok());
                                let permissions_list = try_or_continue!(fetch_all_campsite_permissions(campsite_id, ws_actor).ok());
                                let aggregated = aggregate_ws_permissions(&current_membership.clone().unwrap(), &roles, &permissions_list).await;
...
pub async fn on_reactive_data(omsg: ReactiveSubjectData, ws_actor: &Option<String>, actor_campsites: &mut Vec<Option<String>>, current_campsite: &mut Option<String>, current_membership: &mut Option<CampsiteMember>, permissions: &mut CampsiteMemberPermissions) -> WebSocketOutput {
    ...
    match omsg {
        ...
        ReactiveSubjectData::RocketMessage(msg) => {
            on_rocket_message(msg, ws_actor, actor_campsites, current_campsite, current_membership, permissions).await
        },
        ReactiveSubjectData::MemberRolesModified { campsite_id, actors, role_id, removed, permissions_are_empty, binary } => {
            ...
            // Nothing to modify
            } else if ws_actor.clone().map_or(true, |ws_actor| !actors.contains(&ws_actor)) || current_membership.is_none() {
            ...
            if removed && let Some(current_role_index) = current_membership_unwrapped
                        .roles
                        .iter()
                        .position(|x| x.map_or(false, |x| x == role_id))
            {
                current_membership_unwrapped.roles.remove(current_role_index);
            } else {
            ...
            *current_membership = Some(current_membership_unwrapped.clone());
            ...
            let (_, bonfires_to_update, categories_to_update, tents_to_update) = new_permissions_to_update.unwrap();
        ...
        ReactiveSubjectData::CampsitePermissionUpdated { campsite_id, bonfire_id, category_id, tent_id, user_id, role_id, binary } => {
            ...
            let permissions_result = &fetch_only_specific_permissions(
                ...
                tent_id,
                &ws_actor,
                &current_membership.clone().map_or(
                    vec![],
                    |x| x.roles.clone().iter().filter_map(|&y| y).collect::<Vec<Uuid>>()
                )
            )
                .await;
        ...
        // Bonfire updated, etc.
        ReactiveSubjectData::Bonfire { campsite_id, bonfire_id, binary, deleted } => {
            // Could be used in observer .filter, but borrowing could be less intuitive
            if current_campsite.clone().map_or(true, |current_campsite| current_campsite != campsite_id.clone()) {
                return WebSocketOutput::Ignore;
            }
        ...
        ReactiveSubjectData::Category { campsite_id, bonfire_id, category_id, binary, deleted } => {
            // Could be used in observer .filter, but borrowing could be less intuitive
            if current_campsite.clone().map_or(true, |current_campsite| current_campsite != campsite_id.clone()) {
                return WebSocketOutput::Ignore;
            }
        ...
        ReactiveSubjectData::Tent { campsite_id, bonfire_id, category_id, tent_id, binary, deleted } => {
            // Could be used in observer .filter, but borrowing could be less intuitive
            if current_campsite.clone().map_or(true, |current_campsite| current_campsite != campsite_id.clone()) {
                return WebSocketOutput::Ignore;
            }
        ...
        ReactiveSubjectData::CampsiteGlobal { campsite_id, binary } => {
        ...
        ReactiveSubjectData::CampsiteAdded { campsite_id, to_actor, binary } => {
            if ws_actor.clone().map_or(true, |ws_actor| to_actor.clone() != ws_actor) {
                return WebSocketOutput::Ignore;
            }
        ...
        ReactiveSubjectData::CampsiteRemoved { campsite_id, to_actor, binary } => {
            if ws_actor.clone().map_or(true, |ws_actor| to_actor.clone() != ws_actor) {
                return WebSocketOutput::Ignore;
            } else if current_campsite.clone().map_or(true, |current_campsite| campsite_id.clone() == current_campsite) {
