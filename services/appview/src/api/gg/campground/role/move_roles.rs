...
use campground_lexicon::gg::campground::role::{GetRolesOutput, RoleViewBasic, RolesMovedOutput};
use chrono::Utc;
use diesel::{
...
#[post("/xrpc/gg.campground.role.moveRoles?<campsite_id>", data = "<body>")]
pub async fn move_roles(
    auth: CampsiteInfo<'_>,
    event_subject: &State<ReactiveSubject>,
    campsite_id: &str,
    body: Json<MoveRoleBody>,
    ...
    if roles_by_position.len() < 1 {
        return Err(XRPCError::BadRequest(
            "Expected at least one role provided".to_string(),
        ));
    }
    ...
    if role_not_found.is_some() {
        return Err(XRPCError::BadRequest(format!(
            "Did not find '{}' role in this campsite",
            role_not_found.unwrap().0
        )));
    }
    ...
    if auth.campsite.owner != auth.member.user_id && actor_max_priority <= *max_given_priority {
        return Err(XRPCError::Forbidden(
            "One of role supplied priorities is lower than actor's max role priority".to_string(),
        ));
    }
    ...
    if auth.campsite.owner != auth.member.user_id
        && actor_max_priority <= supplied_roles_max_priority
    {
        return Err(XRPCError::Forbidden(
            "One of the request's roles has lower priority than actor's max role priority"
                .to_string(),
        ));
    }
    ...
    let updated_roles = diesel::update(campsite_role::table)
        .filter(campsite_role::id.eq_any(supplied_role_ids))
        .set((
            // Mandatory
            campsite_role::updatedby.eq(&auth.actor.did),
            campsite_role::updatedat.eq(current_date),
            // Stuff changed
            campsite_role::priority.eq(sql::<Integer>(
                "(\
