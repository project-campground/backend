...
use crate::{
    database::establish_connection, expect_permission, helpers::{api::handle_select_first_error, campsites::campsite_member_view_detailed, permissions::{GeneralPermissionConsts, has_role_perms_or_owner}}, xrpc::{
...
#[post("/xrpc/gg.campground.membership.updateMember?<campsite_id>&<actor>", data = "<body>")]
pub async fn update_member(auth: CampsiteInfo<'_>, campsite_id: &str, actor: &str, body: Json<UpdateMemberBody>) -> Result<Json<CampsiteMemberViewDetailed>> {
    let mut conn = establish_connection().unwrap();

    let required_permission = if auth.actor.did == actor { GeneralPermissionConsts::MANAGE_SELF_IDENTITY } else { GeneralPermissionConsts::MANAGE_OTHERS_IDENTITY };
    
    expect_permission!(has_role_perms_or_owner(&auth.campsite, &auth.member, required_permission, 0));
    ...
    return Ok(Json(campsite_member_view_detailed(&updated_member, &profile, &actor)));
}
