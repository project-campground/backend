use appview_schema::{
    models::appview::{Campsite, CampsiteInvite, CampsiteMember, CampsiteRole},
    schema::appview::{
        self, campsite, campsite_ban, campsite_invite, campsite_member, campsite_role,
    },
};
...
use crate::{
    ...
    views::{campsites::campsite_view_basic, members::member_view_basic, roles::CampsiteRoleFlag},
    xrpc::{
        auth::Authorization,
...
#[post("/xrpc/gg.campground.invite.useInvite?<invite_id>")]
pub async fn use_invite(
    ...
    let (invite, campsite) = campsite_invite::table
        ...
        .inner_join(
            appview::campsite::table.on(appview::campsite::id.eq(campsite_invite::campsiteid)),
        )
    ...
    if actor.campsites.contains(&Some(invite.campsite_id.clone())) {
        return Err(XRPCError::Forbidden(
            "User already joined this campsite".to_string(),
        ));
    } else if invite.allowed_amount.map_or(false, |x| x <= invite.used) {
        return Err(XRPCError::Forbidden(
            "This invite has already hit the allowed use limit".to_string(),
        ));
    } else if invite.expires_at.map_or(false, |x| x >= current_date) {
        return Err(XRPCError::Forbidden(
            "This invite has already expired".to_string(),
        ));
    }
    ...
    let ban_count = campsite_ban::table
        .filter(
            campsite_ban::userid
                ...
                .and(campsite_ban::campsiteid.eq(invite.campsite_id.clone())),
        )
        .count()
        .first::<i64>(&mut conn)
        .map_err(handle_select_first_error)?;
    ...
    if ban_count > 0 {
        return Err(XRPCError::Forbidden(
            "User is banned from this campsite".to_string(),
        ));
    }

    let roles = campsite_role::table
        .filter(campsite_role::campsiteid.eq(invite.campsite_id.clone()))
