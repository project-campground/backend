...
use crate::{
    ...
    util::params::ensure_valid_set_uri,
...
#[post(
    "/xrpc/gg.campground.bonfire.createBonfire?<campsite_id>",
    data = "<body>"
)]
pub async fn create_bonfire(
    auth: CampsiteInfo<'_>,
    event_subject: &State<ReactiveSubject>,
    campsite_id: &str,
    body: Json<CreateBonfireBody<'_>>,
) -> Result<Json<BonfireViewBasic>> {
    let CreateBonfireBody {
        name,
        description,
        position,
        avatar_uri,
