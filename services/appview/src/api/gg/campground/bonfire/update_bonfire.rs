...
use crate::{
    ...
    util::params::{OptionValidity, ensure_valid_modified_uri},
...
#[post(
    "/xrpc/gg.campground.bonfire.updateBonfire?<bonfire_id>",
    data = "<body>"
)]
pub async fn update_bonfire<'a>(
    auth: BonfireInfo<'_>,
    event_subject: &State<ReactiveSubject>,
    bonfire_id: &str,
    body: Json<UpdateBonfireBody<'a>>,
) -> Result<Json<BonfireViewBasic>> {
    let UpdateBonfireBody {
        name,
        description,
        position,
        avatar_uri,
    ...
    let bonfire = diesel::update(appview::bonfire::table)
        ...
        .set((
            appview::bonfire::name.eq(name.clone().unwrap_or(auth.bonfire.name)),
            appview::bonfire::description
                .eq(description.clone().unwrap_or(auth.bonfire.description)),
            appview::bonfire::priority.eq(position.clone().unwrap_or(auth.bonfire.priority)),
