...
use crate::{
    ...
    helpers::{
        ...
        ws::event_next_campsite_global,
    },
    realtime::data::ReactiveSubject,
    util::params::{AsParamValue, OptionValidity, ensure_valid_modified_uri},
...
#[post(
    "/xrpc/gg.campground.campsite.updateCampsite?<campsite_id>",
    data = "<body>"
)]
pub async fn update_campsite(
    ...
    campsite_id: &str,
    body: Json<UpdateCampsiteBody<'_>>,
) -> Result<Json<CampsiteViewBasic>> {
    let UpdateCampsiteBody {
        name,
        description,
        vanity_url,
        tags,
    ...
    let name = &name
        ...
        .map_err(|_| {
            XRPCError::BadRequest(
                "Expected 'name' property to have a string of length 3 to 48 characters"
                    .to_string(),
            )
        })?;
    let description = &description
        ...
        .map_err(|_| {
            XRPCError::BadRequest(
                "Expected 'description' property to have a string of up to 200 characters"
                    .to_string(),
            )
        })?;
    let vanity_url = &vanity_url
        ...
        .map_err(|_| {
            XRPCError::BadRequest(
                "Expected 'vanityUrl' property to have a string of up to 32 characters".to_string(),
            )
        })?;
    let tags = &tags
        .clone()
    ...
    let avatar_uri =
        &ensure_valid_modified_uri(avatar_uri).map_err(|x| XRPCError::BadRequest(x.to_string()))?;
    let banner_uri =
        &ensure_valid_modified_uri(banner_uri).map_err(|x| XRPCError::BadRequest(x.to_string()))?;
    ...
    let campsite = diesel::update(appview::campsite::table)
        ...
        .set((
            appview::campsite::name.eq(name.unwrap_or(auth.campsite.name.as_str())),
            appview::campsite::description
                .eq(description.unwrap_or(auth.campsite.description.as_str())),
            appview::campsite::tags.eq(tags.clone().unwrap_or(auth.campsite.tags)),
            appview::campsite::vanityurl.eq(vanity_url
                .clone()
                .with_fallback(auth.campsite.vanity_url.as_deref())),
            appview::campsite::avataruri.eq(avatar_uri
                .clone()
                .with_fallback(auth.campsite.avatar_uri.as_ref().map(|x| x.as_str()))),
            appview::campsite::banneruri.eq(banner_uri
                .clone()
                .with_fallback(auth.campsite.banner_uri.as_ref().map(|x| x.as_str()))),
            appview::campsite::updatedby.eq(&auth.actor.did),
            appview::campsite::updatedat.eq(current_date),
        ))
