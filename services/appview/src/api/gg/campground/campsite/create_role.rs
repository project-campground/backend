...
use crate::{database::{campsites::get_roles_from_db, establish_connection}, helpers::{api::handle_select_first_error, campsites::campsite_role_view_basic, permissions::{GeneralPermissionConsts, aggregate_member_permissions}, roles::from_role_motion, ws::event_next_campsite}, realtime::data::ReactiveSubject, xrpc::{
...
pub async fn create_role(auth: CampsiteInfo<'_>, event_subject: &State<ReactiveSubject>, campsite_id: &str, body: Json<CreateRoleBody>) -> Result<Json<CampsiteRoleViewBasic>> {    
    let CreateRoleBody { name, motion, colors, display_separately, mentionable, permissions } = &body.into_inner();
