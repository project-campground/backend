...
use crate::{database::establish_connection, expect_permission, helpers::{api::handle_all_db_errors, campsites::bonfire_view_basic, permissions::{GeneralPermissionConsts, has_role_perms_or_owner}, ws::event_next_bonfire}, realtime::data::ReactiveSubject, util::params::ensure_valid_set_uri, xrpc::{
...
pub async fn create_bonfire(auth: CampsiteInfo<'_>, event_subject: &State<ReactiveSubject>, campsite_id: &str, body: Json<CreateBonfireBody<'_>>) -> Result<Json<BonfireViewBasic>> {    
    ...
    expect_permission!(has_role_perms_or_owner(&auth.campsite, &auth.member, GeneralPermissionConsts::MANAGE_BONFIRES, 0));
