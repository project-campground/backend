use appview_schema::{
    models::appview::{Bonfire, Campsite, CampsiteMember, CampsiteRole, Tent},
    schema::appview::actor,
};
...
use crate::{
    ...
    views::{
        bonfires::bonfire_view_basic,
        campsites::campsite_view_detailed,
        members::member_view_basic,
        roles::{CampsiteRoleFlag, role_view_basic},
...
pub async fn create_campsite(
    ...
    body: Json<CreateCampsiteBody<'_>>,
) -> Result<Json<CreateCampsiteOutput>> {
    let CreateCampsiteBody {
        name,
        description,
        vanity_url,
        tags,
    ...
    if name.len() < 3 || name.len() > 48 {
        return Err(XRPCError::BadRequest(
            "Expected 'name' property to have a string of length 3 to 48 characters".to_string(),
        ));
    ...
    let avatar_uri = &ensure_valid_set_uri(avatar_uri)
        .map_err(|x| XRPCError::BadRequest(x.to_string()))?
        .map(|x| x.to_string());
    ...
    if existing_campsite_count >= 20 {
        return Err(XRPCError::Forbidden(
            "Cannot create more than 20 campsites".to_string(),
        ));
    }
    ...
    if vanity_url.clone().map_or(false, |x| {
        ...
        return Err(XRPCError::Forbidden(
            "Cannot use that 'vanityUrl', as a campsite is already using it".to_string(),
        ));
    }

    let (campsite_view, general_tent) = conn
        .transaction(|conn| {
            ...
            diesel::update(actor::table)
                .filter(actor::did.eq(auth.actor_did))
                .set(actor::campsites.eq(new_campsite_list))
                .execute(conn)?;
            ...
            QueryResult::Ok((campsite_view, general_tent))
        })
    ...
    event_next(event_subject, "CampsiteJoined", &campsite_view, |binary| {
        ReactiveSubjectData::CampsiteAdded {
            campsite_id: campsite_view.id.clone(),
            to_actor: actor.did.clone(),
            binary,
        }
    });
    return Ok(Json(CreateCampsiteOutput {
        campsite: campsite_view,
        default_tent: tent_view_basic(&general_tent),
    }));
}
