...
use campground_lexicon::gg::campground::permission::PermissionViewDetailed;
use diesel::{BoolExpressionMethods, ExpressionMethods, QueryDsl, RunQueryDsl};
use rocket::{State, serde::json::Json};
use uuid::Uuid;

use crate::{
    api::gg::campground::permission::update_permission::{
        UpdatePermissionBody, create_or_modify_permission, ensure_update_permission_good_request,
    },
    database::establish_connection,
    helpers::{
        api::handle_select_first_error,
        permissions::{GeneralPermissionConsts, has_full_leveled_perms_from_roles},
    },
    realtime::data::ReactiveSubject,
    views::{
        permissions::permission_view_detailed,
        roles::ensure_no_higher_role,
    },
    xrpc::{
...
pub async fn update_tent_role_permission(
    auth: TentInfo<'_>,
    event_subject: &State<ReactiveSubject>,
    role_id: Uuid,
    body: Json<UpdatePermissionBody>,
) -> Result<Json<PermissionViewDetailed>> {
    ...
    if !(auth.campsite.owner == auth.actor.did
        || has_full_leveled_perms_from_roles(
            &auth.campsite.id,
...
pub async fn update_category_role_permission(
    auth: CategoryInfo<'_>,
    event_subject: &State<ReactiveSubject>,
    role_id: Uuid,
    body: Json<UpdatePermissionBody>,
...
pub async fn update_bonfire_role_permission(
    auth: BonfireInfo<'_>,
    event_subject: &State<ReactiveSubject>,
    role_id: Uuid,
    body: Json<UpdatePermissionBody>,
