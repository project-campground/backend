...
pub async fn unindex_post(
    auth: Authorization<'_>,
    uri: &str,
) -> Result<Json<ProfilePostViewBasic>> {
    ...
    let post_query = profile_post::table
        .filter(profile_post::uri.eq(resolved_uri))
        .first::<ProfilePost>(&mut conn);
    let (actor, main_post) = profile_posts::get_single_profile_post(
        auth.client,
        auth.did_document_storage,
        post_query,
        author_did.as_str(),
    ...
    if auth.actor_did != actor.did {
        return Err(XRPCError::Forbidden(
            "Only author of the post can unindex it".to_string(),
        ));
    }

    delete_post_record_from_db(&main_post.uri, main_post.parent_uri.clone())
        .await
