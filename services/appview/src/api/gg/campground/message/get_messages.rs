...
use campground_lexicon::gg::campground::message::{
    GetMessagesOutput, MessageViewBasic, MessageViewWithReplies,
};
...
use crate::{
    ...
    views::messages::{message_view_basic, message_view_with_replies},
    xrpc::{
        campsite::TentInfo,
...
#[get("/xrpc/gg.campground.message.getMessages?<tent_id>&<limit>&<offset>")]
pub async fn get_messages(
    ...
    if limit < 1 || limit > 100 {
        return Err(XRPCError::BadRequest(
            "Expected limit query to be between and including 1 and 100".to_string(),
        ));
    }
    ...
    if auth.tent.r#type != 0 {
        return Err(XRPCError::BadRequest(
            "Cannot get messages of non-text tents".to_string(),
        ));
    }

    let message_queries: Vec<(
        TentMessage,
        Option<Actor>,
        Option<Profile>,
        ...
        .filter(crate::schema::appview::tent_message::tentid.eq(auth.tent.id))
        .load::<(
            TentMessage,
            Option<Actor>,
            Option<Profile>,
    ...
    let additional_reply_ids = message_queries
        .iter()
        .flat_map(|x| x.0.replying_to.iter().filter_map(|y| y.clone()))
        // Already fetched
        .filter(|x| !message_ids.contains(x))
        .collect::<Vec<Uuid>>();
    // To not re-fetch same messages
    let mut replies = if additional_reply_ids.len() < 1 {
        vec![]
    } else {
        tent_message::table
            .filter(crate::schema::appview::tent_message::id.eq_any(additional_reply_ids))
            .left_join(appview::actor::table.on(tent_message::createdby.eq(appview::actor::did)))
            .left_join(profile::table.on(tent_message::createdby.eq(profile::creator)))
            .left_join(
                campsite_member::table.on(tent_message::createdby
    ...
    return Ok(Json(GetMessagesOutput { messages }));
}
