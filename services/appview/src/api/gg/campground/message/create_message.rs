...
#[post("/xrpc/gg.campground.message.createMessage?<tent_id>", data = "<body>")]
pub async fn create_message<'a>(
    ...
    let (actor, profile) = get_existing_profile_from_actor(client, did_document_storage, auth.actor)
        .await
    ...
    if replies.len() > 5 {
        return Err(XRPCError::BadRequest(
            "Expected no more than 5 elements in 'replies' array".to_string(),
        ));
    }

    let uuids: Vec<Uuid> = replies
        .iter()
        .filter_map(|y| Uuid::try_parse(y.as_str()).ok())
        .collect();

    if uuids.len() < replies.len() {
        return Err(XRPCError::BadRequest(
            "Expected 'replies' parameters to have valid UUIDs".to_string(),
        ));
    }
    ...
    expect_permission!(has_leveled_perms_or_owner(
        ...
        &auth.member,
        0,
        ContentPermissionConsts::VIEW_CONTENT | ContentPermissionConsts::CREATE_CONTENT
    ));
    ...
    let replies_query: Vec<TentMessage> = tent_message::table
        .filter(
            crate::schema::appview::tent_message::tentid
                .eq(auth.tent.id)
                .and(crate::schema::appview::tent_message::id.eq_any(uuids.clone())),
        )
    ...
    if replies_query.len() < uuids.clone().len() {
        return Err(XRPCError::BadRequest(
            "Some of the messages being replied to no longer exist or never existed in this tent."
                .to_string(),
        ));
    }
