...
use crate::{
    database::establish_connection, helpers::{api::handle_all_db_errors, permissions::{ContentPermissionConsts, has_leveled_perms_or_owner}, tents::{tent_message_view_basic, tent_message_view_with_replies}}, xrpc::{
...
pub async fn get_messages(auth: TentInfo<'_>, tent_id: &str, limit: Option<i64>, offset: Option<i64>) -> Result<Json<GetTentMessagesOutput>> {    
    ...
    else if !has_leveled_perms_or_owner(&auth.campsite, &auth.tent.bonfire_id, auth.tent.category_id.clone(), Some(auth.tent.id), &auth.member, 0, ContentPermissionConsts::VIEW_CONTENT).await? {
