...
use crate::{
    database::establish_connection, expect_permission, helpers::{permissions::{ContentPermissionConsts, GeneralPermissionConsts, has_leveled_perms_or_owner}, tents::tent_category_view, ws::event_next_category}, realtime::data::ReactiveSubject, xrpc::{
...
pub async fn delete_category(auth: CategoryInfo<'_>, event_subject: &State<ReactiveSubject>, category_id: &str) -> Result<Json<TentCategoryView>> {
    expect_permission!(has_leveled_perms_or_owner(&auth.campsite, &auth.category.bonfire_id, Some(auth.category.id.clone()), None, &auth.member, GeneralPermissionConsts::MANAGE_TENTS, ContentPermissionConsts::VIEW_CONTENT));
