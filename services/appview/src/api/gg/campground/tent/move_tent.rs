...
use appview_schema::{
    models::appview::{Bonfire, Campsite, CampsiteMember, Tent, TentCategory},
    schema::appview,
};
...
pub async fn move_tent(
    auth: TentInfo<'_>,
    event_subject: &State<ReactiveSubject>,
    tent_id: &str,
    body: Json<MoveTentBody>,
    ...
    if bonfire_id.is_none() && category_id.is_none() && position.is_none() {
        ...
        .clone()
        .map_or(true, |bonfire_id| auth.tent.bonfire_id == bonfire_id)
        || category_id.clone().map_or(true, |category_id| {
            auth.tent
                .category_id
                .map_or("".to_string(), |x| x.to_string())
                == category_id
        })
        || position.map_or(true, |position| auth.tent.priority == position)
    {
    ...
    // Parsed
    let category_id = if category_id.is_none() || remove_category {
        None
    } else {
        Some(
            Uuid::try_parse(category_id.clone().unwrap().as_str()).map_err(|_| {
                XRPCError::BadRequest("Invalid category_id UUID format".to_string())
            })?,
        )
    };
    ...
    let moved_category = if remove_category {
        None
    } else {
        category_id.or(auth.tent.category_id.clone())
    };
    ...
    let updated_tent = diesel::update(crate::schema::appview::tent::table)
        ...
        .set((
            ...
            appview::tent::priority.eq(position.clone().unwrap_or(auth.tent.priority)),
            appview::tent::categoryid.eq(moved_category),
            appview::tent::bonfireid.eq(&moved_bonfire),
            // Mandatory
            appview::tent::updatedat.eq(current_date),
            appview::tent::updatedby.eq(&auth.actor.did),
        ))
        .load::<Tent>(&mut conn)
    ...
    let exists_tents_there = appview::tent::table
        .filter(
            appview::tent::bonfireid
                ...
                .and(appview::tent::categoryid.eq(category_id))
                .and(appview::tent::priority.eq(position)),
        )
        .count()
        .first::<i64>(&mut conn)
        .map_err(handle_select_first_error)?;
        ...
        .filter(
            appview::tent::bonfireid
                ...
                .and(
                    ...
                    appview::tent::categoryid.eq(category_id),
                )
                // Update only tents at that position and below
                .and(appview::tent::priority.ge(position))
                // Make sure it can even go down
                .and(not(appview::tent::priority
                    .cast::<BigInt>()
                    .add(1)
                    .gt(i32::MAX as i64))),
        )
        .set((appview::tent::priority.eq(appview::tent::priority + 1),))
        .load::<Tent>(&mut conn)
        .map_err(handle_select_first_error)?;
...
async fn check_category_existence(
    campsite: &Campsite,
    member: &CampsiteMember,
    moved_bonfire_id: &str,
    category_id: Uuid,
) -> Result<String, XRPCError> {
    let mut conn = establish_connection().unwrap();

    let tent_categories = &crate::schema::appview::tent_category::table
        .filter(crate::schema::appview::tent_category::id.eq(category_id))
        .load::<TentCategory>(&mut conn)
        .map_err(handle_select_first_error)?;

    if tent_categories.len() == 0 {
        return Err(XRPCError::BadRequest(
            "Category supplied in 'category_id' does not exist".to_string(),
        ));
    }
    ...
    if tent_category.campsite_id != campsite.id {
        return Err(XRPCError::BadRequest(
            "Category supplied in 'category_id' does not belong to the same campsite as tent"
                .to_string(),
        ));
...
pub async fn check_bonfire_existence(
    campsite: &Campsite,
    member: &CampsiteMember,
    moved_bonfire_id: &str,
) -> Result<(), XRPCError> {
    ...
    let bonfires = &crate::schema::appview::bonfire::table
        .filter(
            crate::schema::appview::bonfire::id
                .eq(moved_bonfire_id)
                .and(crate::schema::appview::bonfire::campsiteid.eq(&campsite.id)),
        )
    ...
    if bonfires.len() == 0 {
        return Err(XRPCError::BadRequest(
            "Bonfire supplied in 'bonfire_id' does not exist".to_string(),
        ));
    }
