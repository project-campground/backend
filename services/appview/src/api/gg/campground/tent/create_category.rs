...
#[post(
    "/xrpc/gg.campground.tent.createCategory?<bonfire_id>",
    data = "<body>"
)]
pub async fn create_category(
    ...
    let existing_category_count = crate::schema::appview::tent_category::table
        .filter(crate::schema::appview::tent_category::campsiteid.eq(&auth.campsite.id))
        .count()
        .first::<i64>(&mut conn)
    ...
    if existing_category_count >= 25 {
        return Err(XRPCError::Forbidden(
            "Cannot create more than 25 tent categories in a bonfire".to_string(),
        ));
    }
