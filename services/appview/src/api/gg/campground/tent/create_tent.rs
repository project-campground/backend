...
use campground_lexicon::gg::campground::{
    content::{ContentComponent, SystemMessage},
    tent::TentViewBasic,
};
use chrono::Utc;
...
#[post(
    "/xrpc/gg.campground.tent.createTent?<campsite_id>&<bonfire_id>",
    data = "<body>"
)]
pub async fn create_tent(
    ...
    if inner_body.name.len() < 3 || inner_body.name.len() > 48 {
        ...
        return Err(XRPCError::BadRequest(
            "Invalid type. Expected only 0, as other types do not exist yet.".to_string(),
        ));
    } else if inner_body.view_type != 0 {
        return Err(XRPCError::BadRequest(
            "Invalid view type. Expected only 0, as for this type other view types do not exist."
                .to_string(),
        ));
    }

    let category_id = if inner_body.category_id.is_none() {
        None
    } else {
        Some(
            Uuid::try_parse(inner_body.category_id.clone().unwrap().as_str()).map_err(|_| {
    ...
    crate::schema::appview::bonfire::table
        .filter(
            crate::schema::appview::bonfire::id
                .eq(bonfire_id)
                .and(crate::schema::appview::bonfire::campsiteid.eq(campsite_id)),
        )
        .first::<Bonfire>(&mut conn)
        .map_err(handle_select_first_error)?;

    let existing_tent_count = crate::schema::appview::tent::table
        .filter(crate::schema::appview::tent::campsiteid.eq(campsite_id))
        .count()
        .first::<i64>(&mut conn)
    ...
    if existing_tent_count >= 200 {
        return Err(XRPCError::Forbidden(
            "Cannot create more than 200 tents in a campsite".to_string(),
        ));
    }
    ...
    let (_, profile) = get_existing_profile_from_actor(
        &auth.auth.client,
        &auth.auth.did_document_storage,
        auth.actor.clone(),
    )
    .await
