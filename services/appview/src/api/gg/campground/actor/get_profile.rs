...
use crate::{
    ...
    helpers::{
        did::{try_get_did_doc, try_get_pds, try_resolve_actor},
        repo::try_get_record, views::profile_view
    },
...
pub async fn get_profile(auth: UserDidAuthOptional, client: &State<Client>, id_resolver: &State<SharedIdResolver>, actor: &str) -> Result<Json<ProfileView>> {
    let actor = try_resolve_actor(&mut id_resolver.id_resolver.write().await, &actor.to_owned()).await?;
    ...
    return Ok(Json(profile_view(&did_doc, &response.value)))
}