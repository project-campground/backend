...
use rsky_pds::common::get_verification_material;
use rsky_pds::xrpc_server::auth::{verify_jwt as verify_service_jwt_server, ServiceJwtPayload};
use rsky_pds::auth_verifier::{
    AuthScope,
    Credentials,
    AccessOutput,
    ValidatedBearer,
...
use crate::config::{CORE_CONFIG, ENTRYWAY_CONFIG, MOD_SERVICE_CONFIG, SECRET_CONFIG, SERVICE_CONFIG};
...
impl<'r> FromRequest<'r> for UserDidAuth {
    ...
    async fn from_request(req: &'r Request<'_>) -> Outcome<Self, Self::Error> {
        ...
        match verify_service_jwt(
            ...
            id_resolver,
            ServiceJwtOpts {
                aud: Some(SERVICE_CONFIG.did.clone()),
                iss: None,
            },
        )
        .await
...
impl<'r> FromRequest<'r> for ModService {
    ...
    async fn from_request(req: &'r Request<'_>) -> Outcome<Self, Self::Error> {
        let mod_service_config = match MOD_SERVICE_CONFIG.as_ref() {
            Some(config) => config,
            None => {
                return Outcome::Error((
                    Status::BadRequest,
        ...
        if let Some(mod_service_did) = Some(mod_service_config.did.clone()) {
            let id_resolver = req.guard::<&State<SharedIdResolver>>().await.unwrap();
            match verify_service_jwt(
                req,
                id_resolver,
            ...
            {
                Ok(payload)
                    if Some(payload.aud.clone()) != Some(SERVICE_CONFIG.did.clone())
                        && (ENTRYWAY_CONFIG.is_none()
                            || Some(payload.aud.clone()) != Some(ENTRYWAY_CONFIG.clone().unwrap().did)) =>
                {
                    Outcome::Error((
                        Status::BadRequest,
    ...
    async fn from_request(req: &'r Request<'_>) -> Outcome<Self, Self::Error> {
        ...
        match parse_basic_auth(auth_header) {
            ...
            Some(parsed) => {
                ...
                if username != "admin" || password != CORE_CONFIG.admin_pass {
                    Outcome::Error((
                        Status::BadRequest,
