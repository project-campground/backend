...
pub async fn format_url_and_aud<'r>(
    ...
    let default_proxy = default_service(&nsid).await;
    ...
    match (service_url, aud) {
        (Some(service_url), Some(aud)) => {
            ...
            if !CORE_CONFIG.dev_mode() && !is_safe_url(url.clone()) {
...
pub async fn default_service<'r>(
    ...
    match Ids::from_str(nsid) {
        Ok(Ids::ToolsOzoneTeamAddMember) => MOD_SERVICE_CONFIG.clone(),
        Ok(Ids::ToolsOzoneTeamDeleteMember) => MOD_SERVICE_CONFIG.clone(),
        Ok(Ids::ToolsOzoneTeamUpdateMember) => MOD_SERVICE_CONFIG.clone(),
        Ok(Ids::ToolsOzoneTeamListMembers) => MOD_SERVICE_CONFIG.clone(),
        Ok(Ids::ToolsOzoneCommunicationCreateTemplate) => MOD_SERVICE_CONFIG.clone(),
        Ok(Ids::ToolsOzoneCommunicationDeleteTemplate) => MOD_SERVICE_CONFIG.clone(),
        Ok(Ids::ToolsOzoneCommunicationUpdateTemplate) => MOD_SERVICE_CONFIG.clone(),
        Ok(Ids::ToolsOzoneCommunicationListTemplates) => MOD_SERVICE_CONFIG.clone(),
        Ok(Ids::ToolsOzoneModerationEmitEvent) => MOD_SERVICE_CONFIG.clone(),
        Ok(Ids::ToolsOzoneModerationGetEvent) => MOD_SERVICE_CONFIG.clone(),
        Ok(Ids::ToolsOzoneModerationGetRecord) => MOD_SERVICE_CONFIG.clone(),
        Ok(Ids::ToolsOzoneModerationGetRepo) => MOD_SERVICE_CONFIG.clone(),
        Ok(Ids::ToolsOzoneModerationQueryEvents) => MOD_SERVICE_CONFIG.clone(),
        Ok(Ids::ToolsOzoneModerationQueryStatuses) => MOD_SERVICE_CONFIG.clone(),
        Ok(Ids::ToolsOzoneModerationSearchRepos) => MOD_SERVICE_CONFIG.clone(),
        Ok(Ids::ComAtprotoModerationCreateReport) => REPORT_SERVICE_CONFIG.clone(),
        _ => BSKY_APP_VIEW_CONFIG.clone(),
    }
}
