...
pub struct IpldCid(Cid);

impl From<Cid> for IpldCid {
    fn from(cid: Cid) -> Self {
        IpldCid(cid)
    }
}

impl Deref for IpldCid {
    type Target = Cid;
...
impl<'de> Deserialize<'de> for IpldCid {
        ...
        D: serde::Deserializer<'de>,
    {
        ...
        match ipld {
            Ipld::Link(cid) => Ok(IpldCid(cid)),
            Ipld::Bytes(bytes) => {
                let cid = Cid::try_from(bytes).map_err(serde::de::Error::custom)?;
                Ok(IpldCid(cid))
            },
            Ipld::List(list) => {
                if list.len() < 2 {
                    return Err(serde::de::Error::custom("cid must be a link or bytes"));
                }
                let bytes: Vec<u8> = list
                    .iter()
                    .filter_map(|ipld| {
                        if let Ipld::Json(json) = ipld {
                            match json {
            ...
            _ => Err(serde::de::Error::custom("cid must be a link or bytes")),
        }
    }
}
